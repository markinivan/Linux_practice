...
public:
  ...
  void wc_res_print(std::vector<std::pair<std::vector<int>, std::string>> output_table) {
    /*for (int i = 0; i < output_table.size();i++) {
      std::cout << "[";
      for (int j = 0; j < (output_table[i].first).size();j++) {
        std::cout << ((output_table[i].first)[j]) << " ";
      }
      std::cout << "]";
    ...
    for (int i = 0; i < output_table.size();i++) {
      int len = number_len((output_table[i].first)[1]);
      max_chars = max_chars > len ? max_chars : len;
    }
    for (int i = 0; i < output_table.size();i++) {
      int len = number_len((output_table[i].first)[2]);
      max_chars = max_chars > len ? max_chars : len;
    }
    for (int i = 0; i < output_table.size();i++) {
      int len = number_len((output_table[i].first)[3]);
      max_chars = max_chars > len ? max_chars : len;
    }
    for (int i = 0; i < output_table.size();i++) {
